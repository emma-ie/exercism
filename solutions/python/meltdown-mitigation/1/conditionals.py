...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    percent_val = (generated_power / theoretical_max_power) * 100
    if percent_val >= 80:
        return 'green'
    elif 80 > percent_val >= 60:
        return 'orange'
    elif 60 > percent_val >= 30:
        return 'red'
    else:
        return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if temp_and_neutrons < (threshold * 0.9):
        return 'LOW'
    elif temp_and_neutrons >= (threshold * 0.9) and temp_and_neutrons <= (threshold * 1.1):
        return 'NORMAL'
    else:
        return 'DANGER'
